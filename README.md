...
3. Optionally specify a custom module list in JSON format using the
   `UAPI_MODULES` environment variable or by editing `modules.json`.
...
- The module tests in `tests/modules.spec.ts` iterate over modules defined in
  `modules.json` by default. Update that file or set the `UAPI_MODULES`
  environment variable to specify modules for your application.
